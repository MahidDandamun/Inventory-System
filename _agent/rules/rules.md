...
- **Phase 3A: Reporting & Export Center** — Created `lib/dal/reports.ts` for Stock Valuation, Dead Stock, Revenue grouping, Order Fill Rate, and Dashboard metrics. Updated Dashboard page to reflect new real metrics. Built Reports page (`/reports`) with tabbed reports using the `DataTable` export functionality. Added "Reports" to Sidebar and routing.
- **Phase 3B & 3C: Cycle Counts & Governance** — added `CycleCount`, `Lot`, `SerialNumber`, `ApprovalRequest` to schema. Enhanced `SystemLog` with `previousData`/`newData`. Implemented specific Cycle Counts CRUD, UI to schedule counts, mobile-friendly table for logging counts with variance calculations. Designed governance approval engine (`requestStockAdjustmentApproval`) linking significant cycle count variances to admin approval blocks. Built Approvals management page. Added `/cycle-counts` and `/approvals` routes explicitly to sidebar.
